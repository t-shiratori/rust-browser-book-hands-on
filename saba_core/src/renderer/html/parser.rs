...
use super::{attribute::Attribute, token::{HtmlToken, HtmlTokenizer}};
...
impl HtmlParser {
    ...
    fn pop_until(&mut self, element_kind: ElementKind) {
        assert!(self.contain_in_stack(element_kind), "stack does't have an element {:?}", element_kind);
...
mod tests {
    ...
    use crate::renderer::{dom::node::{Element, Node, NodeKind}, html::{attribute::Attribute, token::HtmlTokenizer}};
    ...
    fn text_multiple_nodes() {
        let html = "<html><head></head><body><p><a foo=bar></a>text</p></body></html>".to_string();
        let t = HtmlTokenizer::new(html);
        let window = HtmlParser::new(t).construct_tree();
        let document = window.borrow().document();
        ...
        let text = a.borrow().next_sibling().expect("failed to get a next sibling of a");
        assert_eq!(Rc::new(RefCell::new(Node::new(NodeKind::Text("text".to_string())))), text);
