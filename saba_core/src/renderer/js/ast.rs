...
impl Node {
    ...
    pub fn new_variable_declarator(id: Option<Rc<Self>>, init:Option<Rc<Self>> ) -> Option<Rc<Self>>{
    ...
    pub fn  new_identifier(value: String) -> Option<Rc<Self>> {
        Some(Rc::new(Node::Identifier(value)))
    }
...
pub struct JsParser {
    lexer: Peekable<JsLexer>
}

impl JsParser {
    pub fn new(lexer: JsLexer) -> Self {
    ...
    fn source_element(&mut self) -> Option<Rc<Node>> {
        ...
        match t {
            Token::Keyword(keyword) => {
                if keyword == "function" {
                    assert!(self.lexer.next().is_some());
    ...
    fn parameter_list(&mut self) -> Vec<Option<Rc<Node>>> {    
        ...
        loop {
            // ")" に達するtまで　params　にかり引数となる変数を追加する
    ...
    fn statement(&mut self) -> Option<Rc<Node>> {
        ...
        if let Some(Token::Punctuator(c)) = self.lexer.peek() {
            if c == &';' {
                assert!(self.lexer.next().is_some());
            }
        }
    ...
    fn arguments(&mut self) -> Vec<Option<Rc<Node>>> {
        ...
        loop {
            // ')' に到達するまで、解釈した値を arguments に追加する
            match self.lexer.peek() {
                Some(t) => match t {
                    Token::Punctuator(c) => {
                        if c == &')' {
...
mod tests {
    ...
    use crate::renderer::js::{ast::{JsParser, Node, Program}, token::JsLexer};
