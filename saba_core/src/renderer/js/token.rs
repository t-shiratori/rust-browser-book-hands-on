...
static RESERVE_WORDS: [&str; 3] = ["var", "function", "return"];
...
impl JsLexer {
    ...
    fn contains(&self, keyword: &str) -> bool {
        for i in 0..keyword.len() {
            ...
            .chars()
            .nth(i)
            .expect("failed to access to i-th char") != self.input[self.pos + i] {
                return false
            }
        }
...
impl Iterator for JsLexer {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let token = match c {
            // 記号が現れたら Token::Punctuator トークンを作成して返す
            '+' | '-' | ';' | '=' | '(' | ')' | '{' | '}' | ',' | '.' => {
                let t = Token::Punctuator(c);
                self.pos += 1;
                t
            }
            // 数字が現れたら Token::Number トークンを作成して返す
